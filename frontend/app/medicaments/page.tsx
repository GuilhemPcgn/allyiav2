...
export default function Medicaments() {
  ...
  const handlePrescriptionUpload = async (
    ...
    try {
      const res = await fetch("http://localhost:3000/ordinances", {
        method: "POST",
        body: formData,
      });
      if (res.ok) {
        const data = await res.json();
        const names: string[] = data.medications;
        const meds = names
          .map((n) =>
            allMedicaments.find(
          ...
          <TabsContent value="medicaments" className="mt-6">
            ...
            {selectedMedicaments.length > 0 ? (
              ...
              <p className="text-gray-500">
                Aucun médicament ajouté. Recherchez-en un pour l'ajouter.
              </p>
            )}
          </TabsContent>
